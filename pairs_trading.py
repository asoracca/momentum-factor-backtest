...
def backtest(a, b, prices, beta, mu, sd):
    ...
    for i in range(1, len(prices)):
        ...
        c = abs(pos - prev) * COST
        gross.append(g); net.append(g - c); positions.append(pos)
    idx = prices.index[1:]
    return pd.Series(gross, index=idx).fillna(0.0), pd.Series(net, index=idx).fillna(0.0), positions, z


def main():
    tickers = ["KO", "PEP", "XOM", "CVX", "JPM", "BAC", "HD", "LOW", "V", "MA", "GOOGL", "MSFT"]
    px = yf.download(tickers, start="2017-01-01", auto_adjust=True, progress=False)["Close"].dropna()
    n = len(px); split = int(n * 0.6)
    train, test = px.iloc[:split], px.iloc[split:]
    pairs = list(itertools.combinations(tickers, 2))
    print(f"Loaded {n} days. Train {len(train)} / Test {len(test)}. Scanning {len(pairs)} pairs.\n")
    ...
    print("  -> survives multiple testing." if p_bonf < 0.05 else "  -> does NOT survive multiple testing (likely data-mined).")
    ...
    ntr = sum(1 for j in range(1, len(pos_te)) if pos_te[j] != pos_te[j - 1] and pos_te[j] != 0)

    print("=" * 60)
    print(f"  IN-SAMPLE (train):                  Sharpe {sharpe(n_tr):+.2f}")
    print(f"  OUT-OF-SAMPLE (test, net of costs): Sharpe {sharpe(n_te):+.2f}")
    print(f"     gross Sharpe {sharpe(g_te):+.2f} | trades {ntr} | net return {((1 + n_te).prod() - 1) * 100:+.1f}%")
    print("=" * 60)
    if sharpe(n_te) > 0.5 and p_bonf < 0.05 and ntr >= 10:
        print("  Holds up OOS, after costs, and survives multiple testing — genuinely interesting.")
    else:
        print("  Verdict: collapses once you demand OOS + costs + multiple-testing honesty.")
        print("  That is the realistic outcome for most 'discovered' pairs — and the correct finding.")

    fig, ax = plt.subplots(2, 1, figsize=(10, 7), sharex=True)
    ax[0].plot(z_te.index, z_te)
    for lvl, c in [(2, "r"), (-2, "g"), (0, "gray")]:
        ax[0].axhline(lvl, ls="--", c=c, alpha=.5)
    ...
    ax[1].plot(g_te.index, (1 + g_te).cumprod(), ls="--", alpha=.6, label="gross")
    ax[1].legend(); ax[1].set_title("Out-of-sample cumulative return")
    plt.tight_layout(); os.makedirs("data", exist_ok=True); plt.savefig("data/pairs_oos.png", dpi=110)
